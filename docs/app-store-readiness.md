...
- Development branch/worktree: `codex/app-store-readiness` in the sibling worktree `running_overlay_appstore`.
...
- Network: Open-Meteo historical weather requests and MapKit snapshot requests are outbound only.
...
- Run privacy report / App Store validation with the final dependency graph and signing identity.
