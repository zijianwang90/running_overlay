...
- The Elevation numeric overlay supports `OverlayStyle.elevationDisplayMode` so the same tile can show either live altitude (`current`) or cumulative ascent to the current playhead (`gain`) while keeping the existing meter/feet unit switch. The Temperature numeric overlay supports `OverlayStyle.useFITTemperature`; it defaults on and, when FIT temperature samples exist, exposes an Inspector toggle matching the Weather Widget FIT temperature control.
