...
- Project Settings Font Library persists favorite overlay-menu fonts and the default font family globally; its `Restore Defaults` action returns to the macOS-provided monospaced fallback set (`Menlo`, `PT Mono`, `Monaco`, `Andale Mono`) with `Menlo` as the default.
...
- Numeric Overlay 1.0 supports a configurable SF Symbol icon attachment. `OverlayElementType.defaultNumericIconSystemName` seeds each newly added metric, while `OverlayStyle.iconEnabled`, `iconSystemName`, `iconPosition`, `iconTextAlignment`, `iconSize`, `iconColor`, `iconOpacity`, and `iconSpacing` drive the Inspector and shared SwiftUI preview/export path.
