...
- Left Pool default width is 380 px (min 300 px) and Inspector default width is 400 px (min 380 px); both panels remain user-resizable via custom drag handles.
...
- Inspector default width is 400 px and minimum width is 380 px; the panel is user-resizable through the custom `HorizontalResizeHandle`, and `ParameterPanelView` does not impose its own width frame so internal outer/detail/editing state switches cannot resize the right column or squeeze tile content.
