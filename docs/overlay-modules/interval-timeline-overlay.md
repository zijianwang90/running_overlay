...
- The marker renders just below the segment row, remains inside the background and border, and supports marker color, font size, font weight, editable text, and text visibility controls. The default marker text is `NOW`. Marker arrow, marker text, segment border, and overlay border dimensions are resolved in the shared layout with canvas scaling so 4K export matches preview proportions.
