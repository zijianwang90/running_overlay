...
- Axis labels are configurable separately from Value/Label: a single Enabled toggle shows or hides start, finish, and intermediate labels together; Mode switches start/finish copy vs distance numerals (origin `0 <unit>`); Density sets how many intermediate ticks appear (0 = endpoints only); Point Gap controls vertical offset for all axis labels; font/color/size/weight are shared across axis labels. Preview and export align endpoint labels to the track leading and trailing edges.
