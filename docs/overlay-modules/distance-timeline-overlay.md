...
- Value has its own typography controls, enabled/disabled state, a Total Distance toggle for switching between `current / total unit` and `current unit`, metric/imperial unit system, adjustable Progress Gap to the track, and a Custom Values master toggle. When enabled, all four Custom 1-4 rows appear as metric pickers and render inline after the main value with independent group gap, item gap, size, color, and opacity. Group gap moves the entire custom group without compressing custom text or reducing item gap.
